...
trait LayoutTrait
{
    ...
    {
        // info($this->defaultModel); \App\Models\Post\Post  
        // $fruits = explode("'\'", $this->defaultModel);
        // info($fruits);
    ...
    public function def_edit($uuid){
        ...
        if ($this->isReltionship) {
            ...
            if ($this->paginateRelation) {
                $cardData = $this->defaultModel::with($this->relationName)->where('uuid',$uuid)->first();

                foreach ($this->relationName as $key => $name) {
                   $arr = $cardData->$name()->paginate(config('app.maxRecsPerPage'));
                   $relationships[$name] = $arr;
                   $this->setup[$name] = ucfirst($name) .' List';
                }
    ...
    public function notificationAll($type, $message){
        $notification = null;
        if($type == 'success'){
            $notification = ['type' => 'success', 'message' => $this->pKey == null? config('app.defaultErrors')['crud']['created']:config('app.defaultErrors')['crud']['updated']];
        }
        elseif($type == 'error'){
            $notification = ['type' => 'error', 'message' => $message ? $message : config('app.defaultErrors')['default']];
        }
        elseif($type == 'deleteSuccess'){
            $notification = ['type' => 'success', 'message' => $message ? $message : config('app.defaultErrors')['crud']['deleted']];
        }
        elseif($type == 'generalSuccess'){
            $notification = ['type' => 'success', 'message' => $message ? $message : 'Operation completed successfully'];
        }
        return $notification;
    }
}